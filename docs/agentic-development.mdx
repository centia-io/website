...
description: Learn how to use Centia.io with agentic development.
sidebar_position: 17
---

# Agentic Development with Centia.io

Agentic development is a modern approach to software development where developers use AI agents to build applications through natural language and high-level abstractions. Centia.io is built to be the perfect backend for this workflow, providing the tools and interfaces AI agents need to be productive.

## What is Agentic Development?

Agentic development isn't about writing every line of code yourself. It's about:
...
You can add the Centia.io MCP server to your AI agent (like Claude Code or Junie) using the following configuration:
...
The server shares the configuration with the CLI, so you can just set the connection and login with CLI, and the MCP server is ready. But you can use the server without CLI, so the server accepts the following environment variables:
...
- `API_TOKEN` (optional) — Personal access token for Centia. Most endpoints/tools require authentication; set this to enable them.
...
1. Copy `AGENTS.md` and the `skills/` folder from this [repository](https://github.com/centia-io/mcp-server) to the root of **your own application's** repository.
...
- `centia-map-styling` — Map view configuration and layer cartography (classes, styles, labels)
- `centia-privileges` — Sub-user privileges, groups and rights inheritance
- `centia-openapi-docs` — OpenAPI and docs fallback policy
- `centia-delivery` — Delivery checklist and quality gate

### Key Takeaways for Agents

When an agent is working in an agentic development environment with Centia.io, it follows these priorities:
...
By providing these clear instructions, Centia.io ensures that developers and their agents can build robust, production-ready backends with minimal friction.
