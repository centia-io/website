---
title: SDK
description: Official Centia.io JavaScript/TypeScript SDK with OAuth helpers, SQL and JSON‑RPC clients, and a tiny type‑safe API layer.
sidebar_position: 87
---
...
- Data access:
    ...
    - Keyvalue: Store arbitrary JSON under globally unique keys
...
const flow = new PasswordFlow({
  ...
  database: "parent-database" // The database to connect to. If superuser, this is the same as username.
});

await flow.signIn();
// Tokens are now stored; subsequent SQL/RPC calls include the Authorization header.
...
Execute parameterized SQL against Centia.io. See [SQL](/docs/statement) for more details about the SQL API.
...
Execute GraphQL queries and mutations against Centia.io. See [GraphQL](/docs/graphql) for more details about the GraphQL API.

- Class: `new Gql(schema: string)`
- Method: `query(request: GqlRequest): Promise<GqlResponse>`
- Endpoint: `POST https://api.centia.io/api/graphql/schema/{schema}`

Types (simplified):
- `GqlRequest` has `query`, optional `variables` and `operationName`
- `GqlResponse` has `data` and optional `errors`
...
Store arbitrary JSON under globally unique keys (SDK 0.2.10+). See [Key/value store](/docs/keyvalue) for more details about the Keyvalue API.

- Class: `new Keyvalue(client: CentiaHttpClient)`
- Endpoints: `GET|POST|PATCH|DELETE https://api.centia.io/api/v4/keyvalue/{key}`
...
Types: `KeyvalueEntry<T>` (`{ id, key, value, owner, public }`), `CreateKeyvalueRequest<T>`, `PatchKeyvalueRequest<T>`, `KeyvalueProjection`. Errors are thrown as `CentiaApiError` with `.status` and `.code`.
