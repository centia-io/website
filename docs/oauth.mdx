---
title: OAuth
description: How to use OAuth with Centia.io. Password, Authorization Code (PKCE), and Device flows with examples and best practices.
sidebar_position: 20
---
...
# OAuth
OAuth is an industry-standard protocol for authorization. It enables applications ([clients](./clients)) to
obtain limited access to user accounts on Centia.io, without sharing the user’s credentials
...
Each flow handles authentication and authorization differently.
...
3. Receive token response: If the credentials are valid, the authorization server returns
an access_token and optionally a refresh_token:
...
The [SDK](./sdk) implements the Password Flow with the `PasswordFlow` class. Use it only in trusted environments — the user's credentials are exchanged directly for tokens, which are then stored and refreshed automatically for subsequent SDK calls.
...
- `username`
- `password`
- `database`: The database to connect to. For a super user, this is the same as the username.
...
await flow.signIn();
// Tokens are now stored; subsequent SDK calls include the Authorization header.
...
- Situations where you want to avoid collecting user credentials in the application and
instead rely on a redirect to Centia.io login page.
...
:::note
At the moment, PKCE is mandatory for both public and confidential clients in the Authorization Code Flow.
:::
...
```http title="application/x-www-form-urlencoded"
POST https://api.centia.io/api/v4/oauth
...
The [SDK](./sdk) implements the Authorization Code Flow with PKCE in the `CodeFlow` class — it handles the redirect, `state` validation (CSRF protection), the code/token exchange, token storage and cleanup of the URL for you.

Required options:
- `host`: Base URL of your Centia.io instance, e.g. `https://api.centia.io`
- `clientId`: OAuth client id configured in Centia.io
- `redirectUri`: The URL in your app that handles the redirect back from Centia.io (must be whitelisted on the client)
- `scope` (optional): Not in use yet, but will be used to request additional permissions from the user.

```ts title="Browser SPA"
...
// Start sign-in when the user clicks Login
function onLoginClick() {
  codeFlow.signIn(); // Redirects to the Centia.io auth page
}

// Sign out (clears tokens/options and redirects to the signout endpoint)
...
:::note
`signOut()` clears local tokens/options and redirects to the sign-out URL. If you only need to clear local state without a redirect, call `codeFlow.clear()`.
:::
...
- User Code: AB12-CD34
- Verification URL: https://api.centia.io/device

User authorizes: On a separate device (phone, laptop), the user navigates to https://api.centia.io/device, enters the
...
A guest token gives an app a bearer token for **public access — without any user credentials**. The token is issued for the database's **default user**: the [sub-user](./users.mdx) with `default_user` set, which is the same identity used for anonymous access when no token is sent. Use it when an app should act as the public user against endpoints that require a token.

Request a guest token with the database and an OAuth client id — no `Authorization` header is needed:

```http
POST https://api.centia.io/api/v4/oauth/guest
Content-Type: application/json
...
The response is `201` with the usual token payload:
...
The token carries the same rights as the default user — nothing more.

:::note
- A default user must be configured on the database (`404`, `NO_DEFAULT_USER_FOUND` otherwise).
- `client_secret` is only required for confidential clients; public clients skip the secret check.
- An unknown `client_id` is rejected with `401` (`invalid_grant`); a wrong secret with `401` (`invalid_client`).
:::

### Using the SDK

The [SDK](./sdk) (0.2.14+) wraps the endpoint in the `GuestFlow` class. After `signIn()`, the tokens are stored and all subsequent SDK calls (`Sql`, `Rpc`, `Gql`, ...) are authenticated as the default user, with automatic refresh:
...
await flow.signIn(); // POST /api/v4/oauth/guest — stores the tokens
// ... use Sql, Rpc, Gql etc. as the default user
flow.signOut();
```
...
- Register your client with Centia.io [Client API](./clients) to obtain a client_id and client_secret.
- Configure redirect URIs as needed for each application.
