...
A [scheduler](./import.mdx) job can queue a snapshot automatically after each successful import by setting the job flag `"snapshot": true` through the job API. The flag is API-only for now — the admin UI has no field for it yet.
