...
:::note Authentication and privileges
Which layers a service exposes — and whether they can be read or written — follows the [privileges](./privileges.mdx) of the authenticated user. Anonymous requests are served with the privileges of the default (public) sub-user, so layers readable by the default user are readable without credentials. Protected layers are challenged with HTTP Basic auth on the anonymous endpoints.
:::
