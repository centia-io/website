...
### How it works
1. You **create a JSON-RPC method** by sending a SQL statement with a `method` property giving the method name.
2. You can include:
...
So if you want to create a method that gives you a formatted date plus a given number of days like this:
...
You can create the method without `params`:
...
```http request
POST http://localhost:8080/api/v4/rpc
Content-Type: application/json
Accept: application/json; charset=utf-8
Authorization: Bearer abc123
