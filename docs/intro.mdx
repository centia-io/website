...
- Data you model the way you want
  - Define schemas, tables, columns, and constraints using the Web App, CLI, or API. See Getting started to create your first schema and table. [Read the guide →](./start)
- Realtime APIs
...
- Open, developer‑first tooling
  - A web app for visual management, a CLI for automation, and familiar SQL workflows that fit into your existing toolchain. [Open source →](./opensource)

## Three ways to interact

Centia.io gives you three interfaces — use whichever fits your workflow:

1. **Web App** — A visual interface for managing schemas, tables, users, clients, and more. The quickest way to explore and configure your backend. [Open console →](/console)
2. **CLI** — Automate provisioning from the terminal. Great for scripting, CI/CD, and local development. [Install the CLI →](./start#install-the-cli)
3. **API** — Full HTTP REST and WebSocket access for every operation. Ideal for programmatic control and integration. [API reference →](https://api.centia.io/swagger-ui/index.html)
...
2) Model your data with the Web App, CLI, or API (schemas, tables, columns). [Follow Getting started →](./start)
