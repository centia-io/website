...
:::note
Inheritance applies everywhere privileges are enforced: SQL and the HTTP API as well as the [OGC services](./ogc/services.mdx) (WMS/WFS). The super-user (the database owner) always has full access and is not affected by groups.
:::
